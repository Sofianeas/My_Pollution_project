...
#%%
weather_file_path = 'C:/Users/SCD-UM/OneDrive/Bureau/My_Pollution_project/codes/Mesure_mensuelle_Region_Occitanie_Polluants_Principaux.csv'
df = pd.read_csv(weather_file_path, delimiter=',')
df.head()
#%%
# Describe the 'nom_dept' column
nom_dept_description = df['nom_com'].unique()
nom_dept_description
#%%
# Filter the DataFrame for rows where 'nom_dept' is 'HERAULT'
df_Montpellier = df[df['nom_com'] == 'MONTPELLIER']

# Display the first few rows of the new DataFrame
df_Montpellier.head()
# %%
df_Montpellier.describe()

# %%
# Load the weather data
weather_file_path = 'C:/Users/SCD-UM/OneDrive/Bureau/My_Pollution_project/codes/Weather_forecast_data.csv'
...
df_Montpellier['date_debut'] = pd.to_datetime(df_Montpellier['date_debut'], utc=True)
...
df_Montpellier['date_debut'] = df_Montpellier['date_debut'].dt.tz_localize(None)
weather_data['Date'] = weather_data['Date'].dt.tz_localize(None)

# Set 'date_debut' and 'Date' as the indices
df_Montpellier.set_index('date_debut', inplace=True)
weather_data.set_index('Date', inplace=True)

# Merge the datasets
combined_data = pd.merge(df_Montpellier, weather_data, left_index=True, right_index=True, how='inner')
...
if isinstance(df_Montpellier.index, pd.DatetimeIndex):
    df_HERAULT = df_Montpellier.reset_index()
...
combined_data = pd.merge(df_Montpellier, weather_data, left_on='date_debut', right_index=True, how='inner')
#%%

#%%
# Reset the index to turn 'date_debut' back into a column
...
sub_dataframe = combined_data_reset[relevant_columns]
...
def create_interactive_dual_axis_plot(x, y1, y2, y1_label, y2_label, y1_color, y2_color, title, filename):
    ...
    file_path = f'C:/Users/SCD-UM/OneDrive/Bureau/My_Pollution_project/docs/{filename}.html'
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Température'], sub_dataframe['valeur'],
                                  ...
                                  'Montpellier_Temperature_vs_Pollution_Levels')
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Humidité'], sub_dataframe['valeur'],
                                  ...
                                  'Montpellier_Humidity_vs_Pollution_Levels')
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Vitesse du vent moyen 10 mn'], sub_dataframe['valeur'],
                                  ...
                                  'Montpellier_Wind_Speed_vs_Pollution_Levels')
# %%
# Correlation study : 
...
plt.title('Temperature vs Pollution Value in Hérault')
...
plt.title('Frequency Distribution of Pollution Values in ')
...
pie_chart = px.pie(pollutant_concentration, names='nom_poll', values='valeur', 
                   title="Répartition des concentrations de pollution à Montpellier")
...
# Display the figure
pie_chart.show()
pie_chart.write_html('C:/Users/SCD-UM/OneDrive/Bureau/My_Pollution_project/docs/Montpellier_pie_chart.html',full_html=False, include_plotlyjs='cdn')
# %%
...
fig.update_layout(
    title="Concentration maximale de pollution à Montpellier par mois",
    xaxis_title="Mois",
...
# Display the figure
fig.show()
fig.write_html('C:/Users/SCD-UM/OneDrive/Bureau/My_Pollution_project/docs/Montpellier_bar_chart.html',full_html=False, include_plotlyjs='cdn')
# %%

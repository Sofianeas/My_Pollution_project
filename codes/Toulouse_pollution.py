...
df_TOULOUSE = df[df['nom_com'] == 'TOULOUSE']

# Display the first few rows of the new DataFrame
df_TOULOUSE.head()
# %%
df_TOULOUSE.describe()

# %%
# Load the weather data
weather_file_path = 'C:/Users/SCD-UM/OneDrive/Bureau/My_Pollution_project/codes/Toulouse_meteo.csv'
...
df_TOULOUSE['date_debut'] = pd.to_datetime(df_TOULOUSE['date_debut'], utc=True)
...
df_TOULOUSE['date_debut'] = df_TOULOUSE['date_debut'].dt.tz_localize(None)
weather_data['Date'] = weather_data['Date'].dt.tz_localize(None)

# Set 'date_debut' and 'Date' as the indices
df_TOULOUSE.set_index('date_debut', inplace=True)
weather_data.set_index('Date', inplace=True)

# Merge the datasets
combined_data = pd.merge(df_TOULOUSE, weather_data, left_index=True, right_index=True, how='inner')
...
if isinstance(df_TOULOUSE.index, pd.DatetimeIndex):
    df_HERAULT = df_TOULOUSE.reset_index()
...
combined_data = pd.merge(df_TOULOUSE, weather_data, left_on='date_debut', right_index=True, how='inner')
#%%
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Température'], sub_dataframe['valeur'],
                                  ...
                                  'Toulouse_Temperature_vs_Pollution_Levels')
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Humidité'], sub_dataframe['valeur'],
                                  ...
                                  'Toulouse_Humidity_vs_Pollution_Levels')
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Vitesse du vent moyen 10 mn'], sub_dataframe['valeur'],
                                  ...
                                  'Toulouse_Wind_Speed_vs_Pollution_Levels')
# %%
# Correlation study : 
...
pie_chart = px.pie(pollutant_concentration, names='nom_poll', values='valeur', 
                   title="Répartition des concentrations de pollution à Toulouse")
...
# Display the figure
pie_chart.show()
pie_chart.write_html('C:/Users/SCD-UM/OneDrive/Bureau/My_Pollution_project/docs/Toulouse_pie_chart.html',full_html=False, include_plotlyjs='cdn')
# %%
...
# Display the figure
fig.show()
fig.write_html('C:/Users/SCD-UM/OneDrive/Bureau/My_Pollution_project/docs/Toulouse_bar_chart.html',full_html=False, include_plotlyjs='cdn')
# %%

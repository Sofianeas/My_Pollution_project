...
weather_file_path = 'C:/Users/SCD-UM/OneDrive/Bureau/Project/Projet_Groupe_Pollution_Air_Occitanie/Hautes_Pyrenees_meteo.csv'
...
df_HAUTES_PYRENEES['date_debut'] = pd.to_datetime(df_HAUTES_PYRENEES['date_debut'], utc=True)
...
df_HAUTES_PYRENEES['date_debut'] = df_HAUTES_PYRENEES['date_debut'].dt.tz_localize(None)
weather_data['Date'] = weather_data['Date'].dt.tz_localize(None)

# Set 'date_debut' and 'Date' as the indices
df_HAUTES_PYRENEES.set_index('date_debut', inplace=True)
weather_data.set_index('Date', inplace=True)

# Merge the datasets
combined_data = pd.merge(df_HAUTES_PYRENEES, weather_data, left_index=True, right_index=True, how='inner')
...
if isinstance(df_HAUTES_PYRENEES.index, pd.DatetimeIndex):
    df_HAUTES_PYRENEES = df_HAUTES_PYRENEES.reset_index()
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Température'], sub_dataframe['valeur'],
                                  ...
                                  'Hautes_Pyrenees_Temperature_vs_Pollution_Levels')
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Humidité'], sub_dataframe['valeur'],
                                  ...
                                  'Hautes_Pyrenees_Humidity_vs_Pollution_Levels')
...
create_interactive_dual_axis_plot(sub_dataframe['date_debut'], sub_dataframe['Vitesse du vent moyen 10 mn'], sub_dataframe['valeur'],
                                  ...
                                  'Hautes_Pyrenees_Wind_Speed_vs_Pollution_Levels')
# %%
# Correlation study : 
...
fig = px.pie(pollutant_concentration, names='nom_poll', values='valeur', 
             title="Répartition des concentrations de pollution à l'Hautes Pyrenées")
...
fig.update_layout(
    title="Concentration maximale de pollution à l'Hautes-Pyrenées par mois",
    xaxis_title="Mois",
...
# Display the figure
fig.show()
fig.write_html('C:/Users/SCD-UM/OneDrive/Bureau/Project/Projet_Groupe_Pollution_Air_Occitanie/docs/Hautes_Pyrenées_bar_chart.html',full_html=False, include_plotlyjs='cdn')
# %%
